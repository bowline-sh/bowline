...
const CLAUDE_METADATA_SCAN_LINES: usize = 48;

pub fn discover(options: &HandoffDiscoveryOptions) -> HandoffDiscovery {
    let root = options.claude_home.join("projects");
    let mut candidates = Vec::new();
    let mut skipped = Vec::new();

    for path in discover_jsonl_files(&root) {
        match parse_claude_metadata(&path) {
            Some(metadata)
            ...
            None => skipped.push(SkippedTranscript {
                agent: HandoffAgent::Claude,
                source_path: path,
                reason: "missing Claude session metadata".to_string(),
            }),
        }
    }
...
fn parse_claude_metadata(path: &Path) -> Option<ClaudeMetadata> {
    let lines = read_lines_until_metadata(path, CLAUDE_METADATA_SCAN_LINES).ok()?;
    ...
    for line in lines {
        ...
        cwd = cwd.or_else(|| {
            value
                .get("cwd")
                .and_then(Value::as_str)
                .or_else(|| value.get("projectPath").and_then(Value::as_str))
                .or_else(|| value.get("project_path").and_then(Value::as_str))
                .map(PathBuf::from)
        });
...
pub fn remote_session_relative_path(project_slug: &str, session_id: &str) -> PathBuf {
...
mod tests {
    ...
    #[test]
    fn discovers_matching_claude_transcript() {
        let temp = temp_root("claude-discover");
        let project = temp.join("project");
        fs::create_dir_all(&project).expect("project dir");
        let transcript = temp.join("claude/projects/project/session-1.jsonl");
        fs::create_dir_all(transcript.parent().expect("parent")).expect("sessions dir");
        fs::write(
            &transcript,
            format!(
                "{{\"sessionId\":\"claude_1\",\"cwd\":\"{}\"}}\n{{\"type\":\"assistant\",\"message\":\"KEEP\"}}\n",
                project.display()
            ),
        )
        ...
        assert_eq!(discovery.candidates[0].session_id, "claude_1");
