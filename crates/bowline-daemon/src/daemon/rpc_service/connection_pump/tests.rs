...
use crate::daemon::rpc_service::connection_pump::{MAX_IN_FLIGHT_REQUESTS, run_connection_loop};
use crate::daemon::rpc_service::{RequestRouter, RpcExecutor, rpc_error};
use crate::daemon::{DaemonRuntime, DaemonServerState};
...
#[test]
fn a_malformed_frame_fails_its_own_request_without_killing_the_connection() {
    // Tearing the connection down for one bad frame cancelled the client's other
    // in-flight requests and every status subscription on it, and left the peer
    // with a bare EOF and no reason.
    let state = test_state();
    let executor = test_executor();
    let router: Arc<RequestRouter> =
        Arc::new(|_context, request| success_response(request.request_id));
    let (mut client, server) = start_connection(&state, &executor, router);
    let codec = FrameCodec::default();

    // A frame with a method and a requestId, but an unusable params field.
    codec
        .write(
            &mut client,
            &json!({"requestId": "bad", "method": "daemon.ping", "params": {}, "deadlineMs": "soon"}),
        )
        .expect("malformed frame writes");
    client.flush().expect("malformed frame flushes");
    let rejected = read_until_request(&codec, &mut client, "bad");
    assert_eq!(rejected["error"]["code"], "invalid_request");

    // The connection is still serving.
    write_request(&codec, &mut client, "after", "daemon.ping");
    let served = read_until_request(&codec, &mut client, "after");
    assert!(served["result"].is_object() || served["error"].is_null());
