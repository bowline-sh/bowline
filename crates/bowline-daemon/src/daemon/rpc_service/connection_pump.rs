...
/// Decode and serve one frame.
///
/// A frame this daemon cannot understand is a per-request failure, not a
/// connection failure: tearing the connection down cancels the client's
/// unrelated in-flight requests and every status subscription on it, and leaves
/// the client with a bare EOF. Whenever the frame is well-formed enough to carry
/// a `requestId`, answer with `InvalidRequest` and keep serving. Only genuinely
/// unattributable bytes end the connection, and the reader's own `CodecError`
/// path already covers corrupt framing.
fn handle_payload(payload: &[u8], context: PayloadContext<'_>) -> io::Result<()> {
    let Ok(value) = serde_json::from_slice::<serde_json::Value>(payload) else {
        return write_unattributable_frame_error(
            context.codec,
            context.stream,
    ...
    if value.get("method").is_none() {
        let Ok(cancel) = serde_json::from_value::<DaemonRpcCancel>(value) else {
            return write_unattributable_frame_error(
                context.codec,
                context.stream,
                "the frame is neither a request nor a cancellation",
            );
        };
    ...
    let request_id = value
        ...
        .map(str::to_string);
    let request = match serde_json::from_value::<DaemonRpcRequest>(value) {
        Ok(request) => request,
        Err(error) => {
            let Some(request_id) = request_id else {
                return write_unattributable_frame_error(
                    context.codec,
                    context.stream,
                    "the request frame carries no requestId",
                );
            };
            return write_response(
                ...
                &response_for(
                    ...
                    Err(rpc_error(
                        ...
                        &format!("the request frame is malformed: {error}"),
                        false,
                    )),
                ),
            );
...
/// Report a frame that cannot be attributed to any request. The empty request id
/// tells the client this response belongs to no call it made, while still naming
/// the reason instead of dropping the connection silently.
fn write_unattributable_frame_error(
    codec: FrameCodec,
    ...
    write_response(
        ...
        &response_for(
            String::new(),
            Err(rpc_error(DaemonRpcErrorCode::MalformedFrame, reason, false)),
        ),
    )
}

fn connection_owns_method(method: &str) -> bool {
    super::method_registry::RpcMethod::from_wire(method)
        .is_some_and(super::method_registry::RpcMethod::is_connection_owned)
}
