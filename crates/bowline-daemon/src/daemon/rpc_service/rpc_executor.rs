...
impl RpcLane {
    /// The lane serving a wire method name, or `None` when the method is unknown
    /// or connection-owned. Derived from the one method registry.
    pub(super) fn for_method(method: &str) -> Option<Self> {
        super::method_registry::RpcMethod::from_wire(method)
            .and_then(super::method_registry::RpcMethod::lane)
    }
...
impl RpcExecutor {
    ...
    pub(crate) fn shutdown_strict(
    ...
    ) -> io::Result<super::super::socket_server::ThreadJoinReport> {
        ...
        for worker in workers {
            ...
            match worker.done.recv_timeout(remaining) {
                ...
                Err(RecvTimeoutError::Timeout) => {
                    ...
                    super::super::socket_server::handle_shutdown_grace_expiry("RPC request worker");
        ...
        Ok(super::super::socket_server::ThreadJoinReport {
            expected,
            joined,
            forced_recovery,
        })
