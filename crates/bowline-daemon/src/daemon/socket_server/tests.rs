use crate::daemon::socket_server::{
    AcceptorEvent, BlockingAcceptor, CONNECTION_BUSY_RETRY_AFTER, MAX_CONCURRENT_CONNECTIONS,
    reserve_connection, verify_connection_magic,
};
...
fn overloaded_connection_negotiates_then_returns_typed_retry() {
    ...
    let worker = std::thread::spawn(move || {
        ...
        super::super::rpc_service::reject_overloaded_connection(
            server,
            &server_state,
            None,
...
fn accept_path_has_no_short_poll_or_per_connection_spawn() {
    let socket_server_source = include_str!("../socket_server.rs");
    let executor = include_str!("connection_executor.rs");
    let accept_loop = socket_server_source
    ...
    assert!(!socket_server_source.contains("listener.set_nonblocking(true)"));
