...
/// How often the coordinator checks that the watcher→engine bridge worker is
/// still alive. A worker that exited takes the signal receiver with it and no
/// local edit is observed until it is replaced, so the interval is short; it is
/// an interval rather than an immediate re-arm because the check is the only
/// thing that can tell a live worker from a dead one, and a zero-delay deadline
/// whose handler schedules the next one is a busy loop.
const WATCHER_SUPERVISION_INTERVAL: Duration = Duration::from_secs(1);
...
pub(super) fn run_scheduler(
    ...
    let watcher_bridge = match WatcherBridge::start(&mut runtime) {
        Ok(WatcherBridgeStart::Started(bridge)) => Some(bridge),
        // Both no-bridge outcomes are retried on every engine drive; the watcher
        // one also holds the `ServiceRuntime` status source degraded meanwhile.
        Ok(WatcherBridgeStart::EngineUnavailable | WatcherBridgeStart::WatcherDown) => None,
        Err(error) => {
...
impl SchedulerCoordinator {
    ...
    /// Bring the watcher→engine bridge up if it should be running and is not.
    ///
    /// Called on every engine drive, not only on the driver's first activation:
    /// the watcher kernel can fail to arm at startup (root not yet materialized,
    /// fsevents/inotify budget exhausted) and nothing else in the engine detects
    /// local edits, so the arm attempt has to be a retry loop rather than a
    /// one-shot. Also replaces a bridge whose worker exited — engine death drops
    /// the event sender and kills the previous forwarder.
    fn ensure_watcher_bridge(&mut self, now: Instant) {
        ...
        let started = match self.runtime.lock() {
            Ok(mut runtime) => {
                // The previous bridge worker owns (and drops) the signal
                // receiver, so re-arming rebuilds the kernel to produce a new
                // one. A failed arm logs and schedules its own retry.
                if let Some(sync) = runtime.sync.as_mut() {
                    sync.ensure_watcher_armed(now);
                }
        ...
        match started {
            Ok(WatcherBridgeStart::Started(bridge)) => {
                self.watcher_bridge = Some(bridge);
                self.watcher_workers_started = self.watcher_workers_started.saturating_add(1);
            }
            Ok(WatcherBridgeStart::EngineUnavailable | WatcherBridgeStart::WatcherDown) => {}
            Err(error) => {
                eprintln!(
    ...
    fn schedule_periodic_deadlines(&self, driver: &mut CoordinatorDriver<SystemCoordinatorClock>) {
        ...
        self.schedule_deadline(
            CoordinatorDeadlineKind::WatcherSupervision,
            WATCHER_SUPERVISION_INTERVAL,
            driver,
        );
    }
    ...
    fn drive(
        ...
        // this cycle's status reflects a driver that just came up.
        let now = Instant::now();
        if let Ok(mut runtime) = self.runtime.lock() {
            runtime.retry_manifest_engine(now);
        }
        // Unconditionally, because a bridge can be missing for reasons unrelated
        // to this cycle's driver transition: a watcher that never armed, or one
        // whose forwarder died. Both must keep retrying while the driver stays
        // active, or the daemon reports healthy and sees no local change.
        self.ensure_watcher_bridge(now);
        self.publish_status(executor);
        self.submit_notification(executor);
        self.submit_trust_refresh(executor);
    ...
    fn handle_deadline(
        ...
        match kind {
            ...
            CoordinatorDeadlineKind::WatcherSupervision => {
                self.ensure_watcher_bridge(Instant::now());
                self.schedule_deadline(
                    CoordinatorDeadlineKind::WatcherSupervision,
                    WATCHER_SUPERVISION_INTERVAL,
                    driver,
    ...
    /// Re-arm the engine-retry deadline when any part of the sync runtime is
    /// waiting to retry — a manifest-driver rebuild or a watcher kernel that
    /// could not arm — so the loop wakes at the backoff instant even while
    /// otherwise idle. No deadline is scheduled while both are healthy.
    fn schedule_engine_retry_deadline(
        ...
        let next_retry = self.runtime.lock().ok().and_then(|runtime| {
            runtime
                .sync
                .as_ref()
                .and_then(|sync| sync.next_runtime_retry())
        });
