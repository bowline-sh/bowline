...
use crate::daemon::socket_server::coordinator_runtime::{
    ...
    CoordinatorWorkerOutcome, SchedulerCoordinator, SchedulerFallback, SystemCoordinatorClock,
    WatcherBridge, WatcherBridgeStart, coordinator_channel,
};
use crate::daemon::{
    DaemonRuntime, DaemonServerState, StatusPublishOutcome, StatusPublisher, WatcherSignal,
};
...
fn watcher_bridge_forwards_rename_as_engine_paths() {
    ...
    let WatcherBridgeStart::Started(bridge) =
        ...
        panic!("watcher receiver configured");
    };
...
fn watcher_overflow_collapses_backlog_and_preserves_follow_up_edit() {
    ...
    assert!(matches!(
        engine_rx.recv_timeout(Duration::from_millis(1_800)),
...
fn watcher_bridge_without_engine_does_not_start() {
    ...
    // and the reason must be distinguishable from a dead watcher.
    assert!(matches!(
        WatcherBridge::start(&mut runtime).expect("start ok"),
        WatcherBridgeStart::EngineUnavailable
    ));
    let _ = fs::remove_dir_all(temp);
}
