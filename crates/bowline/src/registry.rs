...
const ENGINE_OPTION: OptionSpec = OptionSpec {
    ...
    summary: "Select the engine to diagnose (default: manifest).",
    required: false,
    repeatable: false,
};
...
fn command_specs() -> impl Iterator<Item = &'static CommandSpec> {
    specs::COMMAND_REGISTRY
        ...
        .chain(doctor_specs::COMMAND_REGISTRY.iter())
}

use internal_specs::INTERNAL_COMMAND_REGISTRY;
...
pub(super) fn print_version(json: bool, socket: &Path) {
    if json {
        print_json(&VersionCommandOutput {
            ...
            // Best-effort daemon probe: omitted (contract: null) when the local
            // daemon is unreachable so harnesses can read the running pair.
            daemon_version: daemon_version(socket),
            platform: Some(bowline_core::introspection::platform_label().to_string()),
            architecture: Some(bowline_core::introspection::architecture_label().to_string()),
